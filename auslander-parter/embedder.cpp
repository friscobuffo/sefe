...
void Embedder::graphAndEmbeddingChecks(const Graph& graph, const Embedding& embedding) const {
    ...
    for (int i = 0; i < graph.size(); ++i) {
        ...
        assert(nodeEmbedding.getNumberOfNeighbors() == node.getNumberOfNeighbors());
    }
}
...
int segmentsMinAttachment[], int segmentsMaxAttachment[]) const {
    for (int i = 0; i < segmentsHandler.size(); i++) {
        ...
        int max = 0;
        for (int j = 0; j < segmentsHandler.getSegment(i).getNumberOfAttachments(); ++j) {
            const Node& attachment = segmentsHandler.getSegment(i).getAttachment(j);
...
int segmentsMinAttachment[], int segmentsMaxAttachment[], const SegmentsHandler& segmentsHandler, int cycleNodePosition) const {
    ...
    for (int i = 0; i < int(maxSegments.size())-1; ++i) {
        ...
        for (int j = i+1; j < maxSegments.size(); ++j) {
            ...
            int numAttachmentsMin = segmentsHandler.getSegment(min).getNumberOfAttachments();
            int numAttachmentsCandidate = segmentsHandler.getSegment(candidate).getNumberOfAttachments();
    ...
    for (int i = 0; i < int(minSegments.size())-1; ++i) {
        ...
        for (int j = i+1; j < minSegments.size(); ++j) {
            ...
            int numAttachmentsMax = segmentsHandler.getSegment(min).getNumberOfAttachments();
            int numAttachmentsCandidate = segmentsHandler.getSegment(candidate).getNumberOfAttachments();
...
const std::vector<std::optional<const Embedding>>& embeddings, const SegmentsHandler& segmentsHandler) const {
    ...
    for (int i = 0; i < segmentsHandler.size(); ++i) {
        ...
        assert(embeddings[i].has_value());
        const Embedding& embedding = embeddings[i].value();
        const Node& attachment = segment.getAttachment(0); // any attachment is good
        ...
        const Node& embeddingAttachment = embedding.getNode(attachmentIndex);
        assert(embeddingAttachment.getNumberOfNeighbors() >= 3);
        for (int j = 0; j < embeddingAttachment.getNumberOfNeighbors(); ++j) {
            const Node& neighbor = embeddingAttachment.getNeighbor(j);
        ...
        assert(position != -1);
        int nextPosition = (position+1)%embeddingAttachment.getNumberOfNeighbors();
        isCompatible[i] = (&segment.getComponentNode(embeddingAttachment.getNeighbor(nextPosition)) != &prev);
    }
    return isCompatible;
}
...
const SubGraph& component, bool compatible, Embedding& output) const {
    ...
    const Node& embeddingNode = embedding.getNode(cycleNodeIndex);
    for (int i = 0; i < embeddingNode.getNumberOfNeighbors(); ++i) {
        const Node& neighbor = embeddingNode.getNeighbor(i);
    ...
    for (int i = 1; i < embeddingNode.getNumberOfNeighbors(); ++i) {
        const int index = (i+positionOfLastAddedNode)%embeddingNode.getNumberOfNeighbors();
        const Node& neighbor = embeddingNode.getNeighbor(index);
        const Node& neighborSegment = segment.getNode(neighbor.getIndex());
        const Node& neighborComponent = segment.getComponentNode(neighborSegment);
...
const std::vector<int>& bipartition) const {
    ...
    for (int i = 0; i < segmentsHandler.size(); ++i) {
        ...
        for (int nodeIndex = 0; nodeIndex < segment.size(); ++nodeIndex) {
            ...
            for (int j = 0; j < embeddingNode.getNumberOfNeighbors(); ++j) {
                const Node& neighbor = embeddingNode.getNeighbor(j);
                const Node& neighborSegment = segment.getNode(neighbor.getIndex());
                const Node& neighborComponent = segment.getComponentNode(neighborSegment);
                neighborsToAdd.push_back(neighborComponent.getIndex());
            }
...
void Embedder::componentAndEmbeddingChecks(const SubGraph& component, const Embedding& embedding) const {
    ...
    for (int i = 0; i < component.size(); ++i) {
        ...
        assert(node.getNumberOfNeighbors() == nodeEmbedding.getNumberOfNeighbors());
    }
}
...
const Embedding Embedder::baseCaseGraph(const Graph& graph) const {
    ...
    for (int nodeIndex = 0; nodeIndex < graph.size(); ++nodeIndex) {
        const Node& node = graph.getNode(nodeIndex);
        for (int i = 0; i < node.getNumberOfNeighbors(); ++i) {
            const Node& neighbor = node.getNeighbor(i);
...
const Embedding Embedder::baseCaseComponent(const SubGraph& component, const Cycle& cycle) const {
    ...
    for (int nodeIndex = 0; nodeIndex < component.size(); ++nodeIndex) {
        ...
        if (node.getNumberOfNeighbors() == 2) { // attachment nodes will be handled later
            embedding.addSingleEdge(node.getIndex(), node.getNeighbor(0).getIndex());
            embedding.addSingleEdge(node.getIndex(), node.getNeighbor(1).getIndex());
            continue;
        }
        assert(node.getNumberOfNeighbors() == 3);
...
const Embedding Embedder::baseCaseCycle(const SubGraph& cycle) const {
    Embedding embedding(cycle);
    for (int i = 0; i < cycle.size()-1; ++i) {
        const Node& node = cycle.getNode(i);
        for (int j = 0; j < node.getNumberOfNeighbors(); ++j) {
            const Node& neighbor = node.getNeighbor(j);
...
public:
    void doCall(ogdf::Graph& graph, ogdf::adjEntry &adjExternal) {
        ...
        for (ogdf::node n : graph.nodes) {
            ...
            for (int i = 0; i < node.getNumberOfNeighbors(); ++i)
                position[node.getNeighbor(i).getIndex()] = i;
            std::vector<ogdf::adjEntry> order(node.getNumberOfNeighbors());
            for (ogdf::adjEntry& adj : n->adjEntries) {
                const int neighbor = adj->twinNode()->index();

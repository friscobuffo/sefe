...
InterlacementGraph::InterlacementGraph(const BlackCycle& cycle, const SegmentsHandler& segmentsHandler)
    ...
    for (int i = 0; i < segmentsHandler_m.size()-1; ++i) {
        ...
        for (int j = i+1; j < segmentsHandler_m.size(); ++j) {
            ...
            for (const Node* attachment : otherSegment.getAttachments()) {
                int attachmentComponent = otherSegment.getHigherLevelNode(*attachment).getIndex();
            ...
            if (areInConflict) addEdge(i, j, 1.0, Color::BLACK);
        }
    }
}

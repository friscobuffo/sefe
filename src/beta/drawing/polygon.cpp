...
bool Line2D::isIntersecting(Line2D& l) {
    ...
    double ub = ((l.p2.x - l.p1.x) * (l.p1.y - p1.y) - (l.p2.y - l.p1.y) * (l.p1.x - p1.x)) / denominator;
...
bool Polygon2D::isOnBoundary(Point2D& p) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (p1.x == p2.x) {
            if (p.x == p1.x && p.y >= std::min(p1.y, p2.y) && p.y <= std::max(p1.y, p2.y)) return true;
        } else {
            double m = (p2.y - p1.y) / (p2.x - p1.x);
            double c = p1.y - m * p1.x;
            double y = m * p.x + c;
            if (p.y == y && p.x >= std::min(p1.x, p2.x) && p.x <= std::max(p1.x, p2.x)) return true;
        }
    }
    return false;
}
...
bool Polygon2D::isInside(Line2D& l) {
    if (!isInside(l.p1) || !isInside(l.p2))
        return false;
    int n = m_points.size();
    for (int i = 0; i < n; i++) {
        Point2D& p1 = m_points[i];
